load('G:\My Drive\_WeldLab\Code\_Analysis\kickedaa\KickedAA_pulseCompare\kickedaa_decay_rates_compare_samples\data\2021-06-17_kickedAA_compareApodizedDecayRates.mat');
load("G:\My Drive\_WeldLab\Code\_Analysis\kickedaa\KickedAA_pulseCompare\kickedaa_decay_rates_compare_samples\data\good_fit_flags_2021-06-17.mat");
RunDatas = Data.RunDatas;

RunDatas([5,14:16]) = [];
good_fit_flags([5,14:16]) = [];
%%
% N = length(Data.RunDatas);
N = length(RunDatas);
...
%     good_fit_tags = flagBadODyFits(RunDatas{ii},'LatticeHold');
    excl_idx = 1:7;
    ...
    try
        fitResult{ii} = kickedAA_decayFit(RunDatas{ii},...
