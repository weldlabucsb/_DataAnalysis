function [ ] = dynaLocalFWH(RunDatas,RunVars,options)
...
varargin = {RunVars.heldvars_all};
    ...
    for j = 1:length(RunDatas)
        ...
        for ii = 1:size(avg_atomdata{j}, 2)
            ...
            options.PlotWidth = 1;
            [width, ~] = fracWidth( X{j}, avg_atomdata{j}(ii).summedODy, frac,'PlotWidth',0);
            if max_ratio > cutoff
                SDys{j}(ii) = NaN;
            else
                SDys{j}(ii) = width;
...
%         fracWidthsvec = [fracWidthsvec fracWidths{j}];
    ...
    figure(1); clf;
    figure_title_dependent_var = ['Cloud FW at .65 max y vs latt hold'];
    hold on;
    for j = 1:length(RunDatas)
...
%     ylim([0,0.07]);
    ...
    for j = 1:3
        ...
        ylim([0 max(Jvals(freqs==freqVals(j)))*1.25]);
    end
