function [outPlot] = pColorCenteredGridLogX(xData,yData,valueData,vectBoundaries,xGrid,yGrid,xTol,yTol)
        ...
        if (abs(xData(ii)-gridPoints(jj,1))<xTol) 
            if (abs(yData(ii)-gridPoints(jj,2))<yTol)
                ...
                if isnan(gridPoints(jj,3))
                    ...
                    warning('An attempt was made to assign more than one value to the grid point with x value %g and y value %g.  The new value was averaged with all other data points on the same grid points.',xGrid(jj),yGrid(jj))
                end
...
% The upper and right edges are all assinged "NaN" because pcolor will ignore them anyway.
...
for ii = 1:length(xGrid)
    for jj = 1:length(yGrid)
        ...
        if ii == 1
            ...
            XCorners(ii,jj) = sqrt(xGrid(ii)*xGrid(ii-1)); % Takes the geometric mean of the two x values
        end
        ...
        if jj==1
            ...
            YCorners(ii,jj) = (yGrid(jj)+yGrid(jj-1))/2;
            ValueCorners(ii,jj) = gridPoints(gg,3);
            gg = gg + 1;
        end
    end
    ...
    if ii == 1
        ...
        XCorners(ii,length(yGrid)+1) = sqrt(xGrid(ii)*xGrid(ii-1)); % Takes the geometric mean of the two x values
    end
...
for jj = 1:length(yGrid)
    ...
    if jj==1
        ...
        YCorners(length(xGrid)+1,jj) = (yGrid(jj)+yGrid(jj-1))/2;
        ValueCorners(length(xGrid)+1,jj) = NaN;
    end
end
...
function vectBoundaries = makeDefaultVectBoundaries(xData,yData)
    ...
    xmin = sqrt((sortedXData(1).^3)/sortedXData(2));
    xmax = sqrt((sortedXData(end).^3)/sortedXData(end-1));
    sortedYData = sort(unique(yData));
    ymin = sortedYData(1) - (sortedYData(2) - sortedYData(1))/2;
    ymax = sortedYData(end) + (sortedYData(end) - sortedYData(end-1))/2;
