...
%             end
            if(RunDatas{j}.vars.Current915ModFreq == 104.8)
                phasonAmp{j}(ii) = (avg_atomdata{j}(ii).PiezoAmp.*4).*445.2/2;
            elseif(RunDatas{j}.vars.Current915ModFreq == 314)
                phasonAmp{j}(ii) = (avg_atomdata{j}(ii).PiezoAmp.*4).*395.41/2;
            elseif(RunDatas{j}.vars.Current915ModFreq == 471)
                phasonAmp{j}(ii) = (avg_atomdata{j}(ii).PiezoAmp.*4).*371.8/2;
            elseif(RunDatas{j}.vars.Current915ModFreq == 942)
                phasonAmp{j}(ii) = (avg_atomdata{j}(ii).PiezoAmp.*4).*336.67/2;
            end
...
%     figure_title_dependent_var = ['cloudSD_y'];
    for j = 1:length(RunDatas)
        plot( varied_var_values{j}, Widths{j}*1E6, 'o-',...
...
%         figure_title_dependent_var = ['cloudSD_y'];
    for j = 1:length(RunDatas)
        plot( phasonAmp{j}, Widths{j}*1E6, 'o-',...
            'LineWidth', options.LineWidth,...
            'Color',cmap(j,:));
%         set(gca,'yscale','log');
        hold on;
    end
    xline(350/2,'r--','linewidth',2);
    xline(804/2,'r--','linewidth',2);

    xline(558/2,'b--','linewidth',2);
    xline(1022/2,'b--','linewidth',2);
    hold off;
...
%             options.Interpreter = 'auto';
    ...
    for j = 1:length(RunDatas)
        ...
        plot( phasonAmp{j}*(4*pi/915), log(Widths{j}./(2.769E-5))./(-0.2071), 'o-',...
            'LineWidth', options.LineWidth,...
            'Color',cmap(j,:));
%         set(gca,'yscale','log');
        hold on;
...
%     xline(1022,'b--','linewidth',2);
    hold off;
                    options.yLabel = 'Effective 915 Strength';
            options.xLabel = {'915 Displacement @ Atoms (nm)'};
%             options.Interpreter = 'auto';
            options.LegendLabels = ["942";"471";"314";"104.8";"$j_{0,1}$";...
                "$j_{0,2}$";"$j_{1,1}$";"$j_{1,2}$"];
