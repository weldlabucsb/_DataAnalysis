function [outPlot] = pColorCenteredGrid(parentAx,xData,yData,valueData,vectBoundaries,xGrid,yGrid,xTol,yTol)
% Used for making a grid of data with rectangular color rectangles on each
% point. 
%
%   parentAx is the Matlab Axes object for the plot.  If that does not
%   sound intuitive to you, make a default figure with the following lines
%   of code:
%      figure()
%      axes()
%   Then set parentAx to gca, i.e. call pColorCenteredGrid(gca,[yourXData],[yourYData],[yourValueData], ...)
%
...
for ii = 1:length(xGrid)
    for jj = 1:length(yGrid)
        ...
        if ii == 1
            ...
            XCorners(ii,jj) = (xGrid(ii)+xGrid(ii-1))/2;
        end
    ...
    if ii == 1
        ...
        XCorners(ii,length(yGrid)+1) = (xGrid(ii)+xGrid(ii-1))/2;
    end
...
outPlot = pcolor(parentAx,XCorners, YCorners,ValueCorners);
